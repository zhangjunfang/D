...
public class SalesController extends BaseWebController {
	...
	public String indexNav(@PathVariable String wid,Model model) {
		logger.error("SalesController index wid:"+wid);
		List<WxArticle> articleList=getArticleList(wid);
		List<WxShop> shopList = getShopList(wid);
		/*Criteria criteria = new Criteria();
		List<WxArticle> list = null;
		List<WxArticle> listTow = null;
		for(int i=0;i<shopList.size();i++){
		criteria.add(Restrictions.eq("shopId", shopList.get(i).getId()));
		criteria.add(Restrictions.eq("categoryName", "即时促销"));
		list=wxArticleService.getList(WxGlobal.DATASOURCE_WEIXIN,shopList.get(i).getId(), criteria);
		//listTow.set(i, list.get(0));
		}*/
		...
		return WxGlobal.WEIXIN_WEB_TEMPLATE+"/wsite/default/sales/sales_navigation";
	}
