...
<div class="c-top"></div>
<div class="container ">
<#list floorList as list>
	<div class="floor clearfix">
		<div class="floor_nav">${list.name}</div>
		<#if list.logoPath?? && list.logoPath!="">
		<div style="width:77px;height:77px;"><img src="${base}${list.logoPath}"/></div>
		</#if>
		<div class="floor_brand">
			<#if brandMap??>
				<#list brandMap?keys as floor>
					<#if floor==list.id>
						<#list brandMap[floor] as brand>
							<div class="brand_img">
							<a href="#"  title="${brand.name}">
							<#if (brand.logoPath != null)!>
								<img src="${base}${brand.logoPath}" alt="${brand.name}"/>
							<#else>
								${brand.name}
							</#if>
							</a>
