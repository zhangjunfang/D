...
<head>
	<meta charset="utf-8" />
	<title>品牌分类列表 - <@spring.message "admin_title"/></title>
	...
	<script type="text/javascript">
    	...
    	$(function (){
    		...
    		$("#dataGrid").ligerGrid({
                columns: [
		            ...
		            { display:'上级分类',name:'parentName', minWidth: 60,width:150, align: 'center' },
    			...
    			url:'${base}/admin/wx/goodscategory/ajax?mid=${(mid)!}',
			...
			$("#goodsCategoryAccordion").ligerAccordion({ height: height, speed: null});
    		 $("#mainTree").ligerTree({  
	            data:${tree}, 
	        	idFieldName :'id',
			...
			accordion = $("#goodsCategoryAccordion").ligerGetAccordionManager();
    	...
    	function onSearchClick(){
            ...
            if(tenantId != "1"){
            	...
            	$.ligerDialog.warn('请选择要查询品牌分类的门店！');
            }
    	...
    	var tenantId = $("#tenantId").val();
    		var wid = $("#wid").val();
    		if(tenantId==1){
				$.ligerDialog.warn('请选择要添加品牌分类的微信号！')
			}else{
		    		$.ligerDialog.open({ 
						url: '${base}/admin/wx/goodscategory/add/'+tenantId+'/'+wid+'?mid=${(mid)!}', 
						width: 400,
						height: 240, 
						title:'添加品牌分类',
						isResize: true,
    	...
    	function onEditClick(){
    		...
    		if(id==""){
				...
				$.ligerDialog.open({ 
					url: '${base}/admin/wx/goodscategory/edit/'+id+'?mid=${(mid)!}', 
					width: 400,
					height: 240,
					title:'修改品牌分类',
					isResize: true,
    	...
    	function onEditDoubleClick(data){
    		$.ligerDialog.open({ 
				url: '${base}/admin/wx/goodscategory/edit/'+data.id+'?mid=${(mid)!}', 
				width: 400,
				height: 240,
				title:'修改品牌分类',
				isResize: true,
				...
				$.ligerDialog.confirm('数据删除后不可恢复,你确认要删除?',function(yes) {
					if(yes){
						$.ajax({
							type:"POST",
							async: true,
							url:"${base}/admin/wx/goodscategory/delete",
...
<body>
	...
	<div id="layout">
        <div position="left" id="goodsCategoryAccordion" style="height:100%;" >
