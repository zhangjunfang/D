...
<head>
	...
	<script type="text/javascript">
		...
		$(function (){
			$("#layout").ligerLayout({
             ...
             $("#actionMenu").ligerMenuBar({items: [
            		<@shiro.hasPermission name="${(msign)!}:${(oper['add'])!}">{ text: '添加分组',click:onAddClick},</@shiro.hasPermission>
	                <@shiro.hasPermission name="${(msign)!}:${(oper['edit'])!}">{ text: '修改分组',click:onEditClick},</@shiro.hasPermission>
            ...
            $("#dataGrid").ligerGrid({
                columns: [
		            ...
		            { display:'分组ID',name:'groupId', minWidth: 60,width:70,align: 'center'},
		            { display:'分组名称',name:'name', minWidth: 60,width:150, align: 'center' },
		            { display:'用户数量',name:'count', minWidth: 60,width:150,align: 'center' },
    			...
    			url:'${base}/admin/wx/followergroup/ajax',
    	...
    	function onAddClick(){
    		...
    		if(tenantId==1){
				$.ligerDialog.warn('请选择要添加分组的微信应用！')
			}else{
				$.ligerDialog.open({ 
					url: '${base}/admin/wx/followergroup/add/'+tenantId+'/'+wid+'?mid=${(mid)!}', 
					width: 380, 
					height: 130,
					title:'添加分组',
					isResize: true,
    	...
    	function onEditClick(){
    		...
    		if(id==""){
				...
				$.ligerDialog.open({ 
					url: '${base}/admin/wx/followergroup/edit/'+id+'?mid=${(mid)!}', 
					width: 380, 
					height: 130,
					title:'修改分组',
					isResize: true,
    	...
    	function onEditDoubleClick(data){
    		$.ligerDialog.open({ 
				url: '${base}/admin/wx/followergroup/edit/'+data.id+'?mid=${(mid)!}', 
				width: 380, 
				height: 130,
				title:'修改分组',
				isResize: true,
    		...
    		$.ligerDialog.confirm('数据更新后不可恢复,你确认要更新?',function(yes) {
					if(yes){
						$.ajax({
							type:"POST",
							async: true,
							url:"${base}/admin/wx/followergroup/download",
        	...
        	<div class="l-panel-search">
			    ...
			    <div class="l-panel-search-item">
			        <select id="property" name="property">
			        	<option value="name" selected>分组名称</option>
			        </select>
			    </div>
			    <div class="l-panel-search-item">
