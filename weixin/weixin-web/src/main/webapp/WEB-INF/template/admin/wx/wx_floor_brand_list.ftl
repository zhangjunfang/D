...
<head>
	<meta charset="utf-8" />
	<title>楼层品牌列表 - <@spring.message "admin_title"/></title>
	...
	<script type="text/javascript">
		...
		$(function (){
            ...
            $("#dataGrid").ligerGrid({
                columns: [
		            ...
		            { display: 'logo图片', name: 'logoPath', minWidth: 60,width:100,heightAlign: 'center',render:function(rowdata){
		            ...
		            { display: '活动图片', name: 'storePath', minWidth: 60,width:100, heightAlign: 'center',render:function(rowdata){
    			...
    			url:'${base}/admin/wx/floorbrand/ajax',
    			parms:[{name:'property', value:''},{name:'keyword', value:''},{name:'mid', value:'${(mid)!}'},{name:'floorId',value:treeId}],
            ...
            $("#mainTree").ligerTree({  
                ...
                onSelect:function(node){ 
            		...
            		if(treeType == "WXFLOOR"){
			            dataGrid.setOptions(
			                { parms:[{ name: 'property', value: property},{ name: 'keyword', value: keyword},{ name: 'tenantId', value: tenantId},{ name: 'floorId', value: treeId}] }
			            );
    	...
    	function onAddClick(){
    		...
    		var treeId = $("#treeId").val();
    		var treeType = $("#treeType").val();
    		if(treeId=="1" || treeType=="CONFIG"||treeType=="WXSHOP"){
				$.ligerDialog.warn('请选择要添加品牌的门店楼层！')
			}else{
				$.ligerDialog.open({ 
					url: '${base}/admin/wx/floorbrand/add/'+tenantId+'/'+treeId+'?mid=${(mid)!}', 
					width: 600, 
					height: 300,
					title:'添加品牌信息',
					isResize: true,
		...
		{
		    $.ajax({
                url: '${base}/admin/wx/floorbrand/deleteLogo',
                type: 'POST',
    	...
    	function onEditClick(){
    		...
    		if(id==""){
				...
				$.ligerDialog.open({ 
					url: '${base}/admin/wx/floorbrand/edit/'+id+'?mid=${(mid)!}', 
					width: 600, 
					height: 300,
					title:'修改品牌信息',
					isResize: true,
    	...
    	function onEditDoubleClick(data){
    		$.ligerDialog.open({ 
				url: '${base}/admin/wx/floorbrand/edit/'+data.id+'?mid=${(mid)!}', 
				width: 600, 
				height: 300,
				title:'修改品牌信息',
				isResize: true,
				...
				$.ligerDialog.confirm('数据删除后不可恢复,你确认要删除?',function(yes) {
					if(yes){
						$.ajax({
							type:"POST",
							async: true,
							url:"${base}/admin/wx/floorbrand/delete",
